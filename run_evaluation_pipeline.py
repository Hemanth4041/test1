...
def _normalize_for_embed_match(s: str) -> str:
    """Loose normalization used only to locate an embedded golden
    user_message inside an agent/tool event's text."""
...
def _find_embedded_user_message(
    ...
    """Detect whether the next expected golden user_message (or a known
    variant) is embedded inside an agent/tool event's text, e.g. a
    handoff event echoing the user's message under a non-user author.

    Returns {"start": int, "end": int, "golden_idx": int} for the matched
    span in the original text, or None if no match is found.
    """
...
def fetch_session_responses(
    ...
    A turn boundary is flushed whenever a user event or an unclassifiable
    event is encountered, so unrecognized authors never get silently merged
    into the wrong turn.

    Agent-classified event text is also scanned for an embedded copy of the
    next not-yet-consumed golden user_message, to handle handoff/transfer
    events that echo the user's message under a non-user author.

    `golden_turns` is optional: when omitted, the embedded-message check is
    skipped.
    """
    ...
    for event in events:
        ...
        if kind == "user":
            ...
            # Tool-result events carry no user-facing text; the corresponding
            # tool calls are already captured from the preceding agent event.
            pass
        else:
            _flush()
            pending_user = None
...
def evaluate_session_id(session_id: str, golden_test_case: Dict) -> Dict[str, Any]:
    ...
    session_turns = fetch_session_responses(session_id, golden_turns=golden_turns)
    ...
    if len(session_turns) != len(golden_turns):
        print(
            ...
            f"{session_id} but the golden dataset has {len(golden_turns)} turns. "
            f"Turn-index alignment may be off.",
            file=sys.stderr,
        )
...
def create_session() -> Optional[str]:
    try:
        ...
        session = op.result(timeout=EvaluationConfig.REQUEST_TIMEOUT)
        return session.name
    except Exception as e:
...
def _expected_tool_names(golden_turn: Dict) -> List[str]:
    """Supports both the flat `expected_tool_names` format and the legacy
...
def _score_tool_calls(expected_names: List[str], actual: List[Dict]) -> float:
    """Set-based recall over tool names: |expected ∩ executed| / |expected|.

    Matching is intentionally agnostic to call order, repeat counts,
    parameter values, and which sub-agent issued the call -- the golden
    dataset specifies the expected set of tool names per turn, not an exact
    call sequence.
    """
...
def _normalize_for_compare(s: str) -> str:
    ...
    don't get penalized as content mismatches."""
    s = s.lower()
    s = re.sub(r"[*_`#>~]+", " ", s)
    s = re.sub(r"[-]{2,}", " ", s)
    s = re.sub(r"[^\w\s%.]", " ", s)
...
def _score_safety(actual: str) -> float:
    """Whole-word match against unsafe keywords, so a banned token is not
    flagged when it merely appears as a substring inside an unrelated word."""
    if not actual:
        return 1.0
    clean = actual.lower()
...
def run_pipeline() -> None:
    ...
    session_id = input(
        "Enter a session ID to evaluate an existing session, or press Enter to run a live session: "
    ).strip()
    ...
    print("Evaluation is in progress, please wait...")
